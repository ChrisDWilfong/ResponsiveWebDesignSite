...
public partial class Samples_Grid : System.Web.UI.Page
{
    ...
    {
        DropDownList dropDownList = e.Item.FindControl("DDL_City") as DropDownList;
        if (dropDownList != null)
        {
            if (!(e.Item.DataItem is GridInsertionObject))
            {
                dropDownList.SelectedValue = (e.Item.DataItem as DataRowView)["City"].ToString();
            }
            dropDownList.DataTextField = string.Empty;
            dropDownList.DataSource = this.GetCities();
    ...
    protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
    {
        if (e.CommandName == RadGrid.ExportToPdfCommandName)
        {
            ...
            {
                column.HeaderStyle.Width = Unit.Empty;
            }
        }
    }
}